 ...
 * @author Chris Caron
 */
enum class MoveType(val aiUseOnce: Boolean, val priority: Int, val _nameId: String, val helpTextId: String) : ILocalized {
	...
	// roll die/event
	INVALID(false, -1, "", ""),
	CONTINUE(false, 0, "End Turn", "End your turn and allow next player to play."),  // use a soldier
	SOLDIER_CARD(true, 50, "Soldier", "Move the robber or Pirate to cell of players choice"),  // seafarers scenario pirate islands
	WARSHIP_CARD(false, 50, "Warship", "Turn one of your ships into a warship"),  // Use a monopoly card
	MONOPOLY_CARD(true, 50, "Monopoly", "Choose a Resource and all opponents give you that resource from their hand"),  // Use a year of plenty card
	YEAR_OF_PLENTY_CARD(true, 0, "Year of Plenty", "Choose 2 Resources to add to your hand"),  // view trade options
	TRADE(false, 1, "Trade", "View your trade options"),  // Draw a card
	DRAW_DEVELOPMENT(false, 50, "Development", "Draw random Development Card for cost of %s") {
		override fun getHelpText(rules: Rules): String {
			return String.format(helpTextId, BuildableType.Development.niceString)
		}
	...
	MOVE_SHIP(true, 5, "Move Ship", "Reposition an open ended ship"),  // use a road building card
	ROAD_BUILDING_CARD(true, 3, "Road Building", "Build two Routes for free"),  // Build a ship
	BUILD_SHIP(false, 5, "Build Ship", "Position a new ship for cost of %s") {
		override fun getHelpText(rules: Rules): String {
			return String.format(helpTextId, BuildableType.Ship.niceString)
		}
	},
	BUILD_WARSHIP(false, 50, "Build Warship", "Upgrade an existing ship to a war ship for cost of %s. Warships can chase away the pirate and engage other players ships.") {
		override fun getHelpText(rules: Rules): String {
			return String.format(helpTextId, BuildableType.Warship.niceString)
		}
	},
	ATTACK_SHIP(false, 0, "Attack a ship", "Your warship can attack an opponent's ship. Roll a die. If 1,2,3 then opponent wins and you lose your warship.  On a 4,5,6 you win and opponent ship becomes yours.  The midpoint of the die is shifted based on the difference in number of cities possessed be each player."),  // Roll dice and take a chance at converting the fortress to a settlement.  Must have a ship adjacent to the settlement.
	ATTACK_PIRATE_FORTRESS(false, 50, "Attack Fortress", "Roll the dice and take a chance at converting the fortress to a new settlement. Three wins converts the settlement. A loss makes you lose the 2 ships adjacent to the fortress. A win costs you 1 ship"),  // Build a Road
	BUILD_ROAD(false, 5, "Build Road", "Position a new road for cost of %s") {
		override fun getHelpText(rules: Rules): String {
			return String.format(helpTextId, BuildableType.Road.niceString)
		}
	...
	BUILD_SETTLEMENT(false, 0, "Build Settlement", "Position a new Settlement for cost of %s") {
		override fun getHelpText(rules: Rules): String {
			return String.format(helpTextId, BuildableType.Settlement.niceString)
		}
	},  // Build a City
	BUILD_CITY(false, 0, "Build City", "Convert a settlement to a City for cost of %s") {
		override fun getHelpText(rules: Rules): String {
			return String.format(helpTextId, BuildableType.City.niceString)
		}
	...
	ROLL_DICE_NEUTRAL_PLAYER(false, 100, "Roll dice", "Roll for the neutral player. Dice outcome will not be same as first roll."),  // used unstead of dice for TAB expansion
	DEAL_EVENT_CARD(false, 100, "Deal Event", "Turn over the next event card"),
	DEAL_EVENT_CARD_NEUTRAL_PLAYER(false, 100, "Deal Event", "Turn over the next event card for the neutral player"),  // CAK Moves
	BUILD_CITY_WALL(false, 10, "Build Wall", "Build a wall around one of your cities for cost of %s. Wall be destroyed when barbarians win an attack but city is reserved") {
		override fun getHelpText(rules: Rules): String {
			return String.format(helpTextId, BuildableType.CityWall.niceString)
		}
	},
	...
	IMPROVE_CITY_TRADE(false, 0, "Improve Trade", "Exchange Cloth to improve city trade.Upgrade to Bank converts to a Metropolis if not taken>"),  // Science
	...
	MINING_CARD(true, 50, "Mining", "Collect 2 ore card for each structure adjacent to a ore tile"),  // Politics
	...
	WEDDING_CARD(true, 0, "Wedding", "All players with more points give you any 2 commodity or resource cards of their choice"),  // Trade
	...
	TRADE_MONOPOLY_CARD(true, 0, "Trade Monopoly", "All players give you 1 commodity of your choice if they have it"),  // Knight actions
	HIRE_KNIGHT(false, 10, "Hire Knight", "Position new basic inactive knight for cost of %s") {
		override fun getHelpText(rules: Rules): String {
			return String.format(helpTextId, BuildableType.Knight.niceString)
		}
	},
	ACTIVATE_KNIGHT(false, 50, "Activate Knight", "Activate one of your knights for cost of %s") {
		override fun getHelpText(rules: Rules): String {
			return String.format(helpTextId, BuildableType.ActivateKnight.niceString)
		}
	},
	PROMOTE_KNIGHT(false, 40, "Promote Knight", "Promote one of your knight for cost %s") {
		override fun getHelpText(rules: Rules): String {
			return String.format(helpTextId, BuildableType.PromoteKnight.niceString)
		}
	},
	...
	KNIGHT_ATTACK_ROAD(false, 50, "Attack Road", "Pick a road adjacent to the knight and roll a die. If the die roll+knight level is greater than or equal to %d then the road is removed from the board") {
		override fun getHelpText(rules: Rules): String {
			return String.format(helpTextId, rules.knightScoreToDestroyRoad)
		}
	},
	KNIGHT_ATTACK_STRUCTURE(false, 50, "Attack Structure", "") {
		override fun getHelpText(rules: Rules): String {
			if (rules.knightScoreToDestroySettlement > 0) {
				return String.format("If knight level+die roll is greater than or equal to %d, then settlement destroyed", rules.knightScoreToDestroySettlement)
			}
			if (rules.knightScoreToDestroyCity > 0) {
				return String.format("If knight level+die roll is greater than or equal to %d, then city reduced to settlement", rules.knightScoreToDestroyCity)
			}
			if (rules.knightScoreToDestroyWalledCity > 0) {
				return String.format("If knight level+die roll is greater than or equal to %d, then wall removed", rules.knightScoreToDestroyWalledCity)
			}
			return if (rules.knightScoreToDestroyMetropolis > 0) {
				String.format("If knight level+die roll is greater than or equal to %d, then metropolis reduced to city", rules.knightScoreToDestroyMetropolis)
			} else ""
		}
	};

	override fun getNameId(): String = String.format(_nameId)

	open fun getHelpText(rules: Rules): String {
