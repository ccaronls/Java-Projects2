...
open class CCActivityBase : AppCompatActivity() {
	...
	protected open fun onPermissionLimited(permissionsNotGranted: List<String>) {
		newDialogBuilder().setTitle("Cannot Launch")
			...
			.setNegativeButton(R.string.popup_button_ok) { dialogInterface: DialogInterface?, i: Int -> finish() }.show()
	...
	fun showEditTextInputPopup(
		title: String?,
		defaultValue: String?,
		hint: String?,
		maxChars: Int,
		callabck: (String) -> Unit
	) {
		val et = EditText(this)
		et.hint = hint
	...
	val externalStorageDirectory: File
		get() {
			...
			return getSdCardPaths(this@CCActivityBase, true)
				?.firstOrNull()?.toFile() ?: getExternalFilesDir(null) as File
		}
