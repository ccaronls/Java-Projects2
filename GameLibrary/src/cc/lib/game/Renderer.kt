...
open class Renderer(
	...
	fun multiply(m: Matrix3x3) {
		Matrix3x3.multiply(cur_mat, m, cur_mat)
	}
	...
	fun clearBoundingRect() {
		_min.assign(Float.MAX_VALUE, Float.MAX_VALUE)
		_max.assign(-Float.MAX_VALUE, -Float.MAX_VALUE)
	}
	...
	private val m_stack: Array<Matrix3x3>
	private var cur_mat: Matrix3x3
	var stackSize: Int
		private set
	private val m_stack_proj: Array<Matrix3x3>
	private var proj_mat: Matrix3x3
	...
	private val _min = MutableVector2D()
	val min: Vector2D
		get() = _min
	private val _max = MutableVector2D()
	val max: Vector2D
		get() = _max
	...
	init {
		pts = Array(MAX_VERTS) { MutableVector2D() }
		names = IntArray(MAX_VERTS) { 0 }
		m_stack = Array(MATRIX_STACK_SIZE) { Matrix3x3() }
		stackSize = 1
		cur_mat = m_stack[0]
		m_stack_proj = Array(MATRIX_STACK_SIZE) { Matrix3x3() }
