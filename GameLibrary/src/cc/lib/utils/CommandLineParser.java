...
public class CommandLineParser {
    ...
    private enum Type {
        ...
        static Type getParam(char c) {
            ...
            throw new IllegalArgumentException("Invalid Type specification [" + c + "], must be one of [" + getAllParams() + "]");
        }
    };
    ...
    private HashMap<Character, Param> params = new LinkedHashMap<Character, Param>();
    ...
    private String parseParam(String format) {
        ...
        if (!Character.isLetter(c))
            throw new IllegalArgumentException("Invalid params format at index [" + formatIndex + "], expected letter");
     ...
     * @throws IllegalArgumentException if an invalid parameter is found
     */
    public void parse(String [] argv, String format) throws IllegalArgumentException {
        int i=0;
        Param param = null;
        ...
        try {
            for (i=0; i<argv.length; i++) {
                if (argv[i].startsWith("-")) {
                    ...
                    if (param == null)
                        throw new IllegalArgumentException("Invalid option [" + argv[i] + "]");
    		...
    		throw new IllegalArgumentException("Not a file [" + e.getMessage() + "]");
    	} catch (NumberFormatException e) {
    		throw new IllegalArgumentException("Not a number [" + e.getMessage() + "]");
    	} catch (Exception e) {
    ...
    private String parseArgFormat(String format) {
    	char c = format.charAt(0);
    	if (!Character.isLetter(c))
    		throw new IllegalArgumentException("Invalid format at index " + formatIndex + ", expected letter, found [" + c + "]");
    	...
    	if (Character.isUpperCase(c)) {
    		...
    		throw new IllegalArgumentException("Invalid format at index " + formatIndex + ", cannot specify required parms after non-required");
    	}
    ...
    private void parseArgs(String [] argv, int index, String format) throws Exception {
    	...
    	while (index < argv.length) {
    		if (argsIndex >= args.size())
    			throw new IllegalArgumentException("Too may arguments");
    	...
    	if (index<args.size()) {
    		if (args.get(index).required)
    			throw new IllegalArgumentException("Not enough arguments specified.  Expected " + args.get(index).type + " [" + args.get(index).desc + "]");	        }
    ...
    public boolean getParamSpecified(char c) {
        ...
        if (p == null)
            throw new IllegalArgumentException("'" + c + "' is not a valid param, is one of " + params.keySet());
    ...
    public void printUsage(Class<?> clazz) {
    	printUsage(clazz.getSimpleName());
    }
    ...
    public void printUsage(String appName) {
    	System.out.println(getUsage(appName));
    }
     ...
     * @return
     */
    public String getUsage(Class<?> clazz) {
    	return getUsage(clazz.getSimpleName());
    }
