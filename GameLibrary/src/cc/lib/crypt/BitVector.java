 ...
 *
 */
public final class BitVector extends Reflector<BitVector> {

	static {
		addAllFields(BitVector.class);
	}
    ...
    public boolean get(int index) {
        int _index = index + begin;
        if (index >= len)
            throw new IndexOutOfBoundsException(String.valueOf(index));
        return (buffer[(int)(_index>>5)] & (1<<(_index&0x1f))) != 0; // optimized
    }
    ...
    public void set(int index, boolean value) {
        ...
        if (index < 0 || index >= this.getMaxBits())
            throw new IndexOutOfBoundsException(String.valueOf(index));
        if (!value)
            buffer[(int)(_index>>5)] &= ~(1<<(_index&0x1f));
        else
            buffer[(int)(_index>>5)] |= (1<<(_index&0x1f));
    }
    ...
    private void privShiftRight(int numBits) {
