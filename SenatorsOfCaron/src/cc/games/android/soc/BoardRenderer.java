...
public class BoardRenderer extends BaseRenderer {
    ...
    interface PickHandler {
    	 ...
    	 * @param pickedValue
    	 */
    	void onPick(BoardRenderer bc, int pickedValue);
    	 ...
    	 * @param index
    	 */
    	void onDrawPickable(BoardRenderer bc, AGraphics g, int index);
    	 ...
    	 */
    	void onDrawOverlay(BoardRenderer bc, AGraphics g);
    	 ...
    	 * @param highlightedIndex
    	 */
    	void onHighlighted(BoardRenderer bc, AGraphics g, int highlightedIndex);
    	 ...
    	 * @return
    	 */
    	boolean isPickableIndex(BoardRenderer bc, int index);
    }
	...
	int waterImage;
	int goldImage;
	int undiscoveredImage;
	int robberImage;
	int pirateImage;
	// CAK extension
	int foresthexImage;
	int hillshexImage;
	int mountainshexImage;
	int pastureshexImage;
	int fieldshexImage;
	
	int cardFrameImage;

    final int roadLineThickness = 4;
	final int bkColorARGB = Color.LTGRAY;
	final int outlineColorARGB = Color.BLACK;
	...
	BoardRenderer(GLSurfaceView parent) {
		super(parent);
	}
	...
	private HashMap<FaceType, Face[]> faceMap = new HashMap<BoardRenderer.FaceType, BoardRenderer.Face[]>();
	...
	void drawFaces(AGraphics g, IVector2D pos, float angle, float w, float h, FaceType structure, boolean outline) {
	    ...
	    for (Face face : faces) {
	    	...
	    	if (outline) {
	    	    g.setColorARGB(outlineColorARGB);
	    	    g.drawLineLoop(2);
	    	}

	    	AColor c = g.makeColorRGBA(face.added).add(g, saveColor.darken(g, face.darkenAmount));//AWTUtils.addColors(face.added, AWTUtils.darken(saveColor, face.darkenAmount));
            g.setColor(c);
            g.drawTriangleFan();
	    }
    ...
    private int pickTile(float X, float Y) {
        Board board = getBoard();
        int picked = -1;
        float x = X / getWidth();
        float y = Y / getHeight();
        float d = (board.getTileWidth() + board.getTileHeight()) / 4;
        d *= d;
    ...
    private int pickEdge(float X, float Y) {
        Board board = getBoard();
        int picked = -1;
        float x = X / getWidth();
        float y = Y / getHeight();
        ...
        for (int i=0; i<board.getNumRoutes(); i++) {
        	if (pickHandler.isPickableIndex(this, i)) {
                ...
                float d2 = Utils.distSqPointSegment(x, y, v0.getX(), v0.getY(), v1.getX(), v1.getY());
                if (d2 < 100) {
                    if (picked == -1 || d2 < d) {
                        picked = i;
                        d = d2;
    ...
    private int pickVertex(float X, float Y) {
        Board board = getBoard();
        int picked = -1;
        float x = X / getWidth();
        float y = Y / getHeight();
        ...
        for (int i=0; i<board.getNumVerts(); i++) {
        	if (pickHandler.isPickableIndex(this, i)) {
                Vertex v = board.getVertex(i);
                float d2 = Utils.distSqPointPoint(x, y, v.getX(), v.getY());
                if (d2 < 500) {
                    if (picked == -1 || d2 < min) {
                        picked = i;
                        min = d2;
	...
	public void drawRobber(AGraphics g, Tile cell) {
        ...
        if (!getRenderFlag(RenderFlag.DONT_DRAW_TEXTURES))
            g.drawImage(robberImage, x, y, w,h);
        else {
            g.setColor(g.BLACK);
            g.fillOval(x,y,w,h);
        }*/
	...
	public void drawPirate(AGraphics g, IVector2D v) {
        ...
        //int y = Math.round(sy * cell.getY()) - h/2;
        if (!getRenderFlag(RenderFlag.DONT_DRAW_TEXTURES))
            g.drawImage(pirateImage, x, y, w,h);
        else {
            g.setColor(g.BLACK);
            g.fillOval(x,y,w,h);
        }*/
	...
	public void drawIslandInfo(AGraphics g, Island i) {
    	...
    	String txt = "ISLAND\n" + i.getNum();
    	GDimension dim = DroidUtils.computeTextDimension(g, txt);
    	g.setColor(g.makeColor(0,0,0,0.5f));
	...
	public void setTouchPosition(int mx, int my) {
		int index = -1;
		switch (pickMode) {
    		case PM_NONE: 
    		    break;
    		case PM_EDGE:
    			index = pickEdge(mx, my);
    			break;
    		case PM_VERTEX:
    		    index = pickVertex(mx, my);
    			break;
    		case PM_TILE:
    		    index = pickTile(mx, my);
    			break;
    		case PM_CUSTOM:
    			index = ((CustomPickHandler)pickHandler).pickElement(mx, my);
    			break;
		}
        ...
        if (getRenderFlag(RenderFlag.SHOW_CELL_INFO)) {
            cellInfoIndex = pickTile(mx, my);
        }

        if (getRenderFlag(RenderFlag.SHOW_EDGE_INFO)) {
            edgeInfoIndex = pickEdge(mx, my);
        }

        if (getRenderFlag(RenderFlag.SHOW_VERTEX_INFO)) {
            vertexInfoIndex = pickVertex(mx, my);
        }
    ...
    private void drawVertexInfo(AGraphics g, int vertexIndex) {
        ...
        MutableVector2D v = new MutableVector2D(vertex);
        g.transform(v);
