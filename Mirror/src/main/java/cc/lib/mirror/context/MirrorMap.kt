...
inline fun <reified K, reified V> Map<K, V>.toMirroredMap(): MirrorMap<K, V> {
	if (this is MirrorMap)
		return this
	return MirrorMap(toMutableMap(), K::class.java, V::class.java)
}

/**
 * Created by Chris Caron on 12/10/23.
 */
class MirrorMap<K, V>(map: Map<K, V>, keyType: Class<K>, valueType: Class<V>) : Mirrored, MutableMap<K, V> {

	private val map = map.toMutableMap()
	private var sizeChanged = false
	private val changedKeys = mutableSetOf<K>()
