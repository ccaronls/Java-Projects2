		...
		fun miniMaxR(game: Game, root: Move?, maximizePlayer: Boolean, depth: Int, actualDepth: Int): Long {
			...
			return if (maximizePlayer) {
				...
				root.children?.forEach { m ->
					...
					val v =
						miniMaxR(game, m, sameTurn, if (sameTurn) depth else depth - 1, actualDepth + 1)
					m.bestValue = v
					if (v > value) {
						path = m
						value = v
...
negamax(rootNode, depth, −1)
	...
	fun buildMovesList(_game: Game) {
		...
		lastSearchResult =
			Move(MoveType.END, game.turn)
		var root = lastSearchResult
		try {
			when (algorithm) {
				Algorithm.minimax -> root!!.bestValue =
					miniMaxR(game, root, true, maxSearchDepth, 0)

				Algorithm.miniMaxAB -> root!!.bestValue =
					miniMaxABR(game, root, true, maxSearchDepth, 0, Long.MIN_VALUE, Long.MAX_VALUE)

				Algorithm.negamax ->                     //                root.bestValue = negamaxR(game, root, -1, maxSearchDepth, 0);
					root!!.bestValue =
						negamaxR(game, root, if (game.turn != 0) 1 else -1, maxSearchDepth, 0)

				Algorithm.negamaxAB -> root!!.bestValue =
					negamaxABR(game, root, 1, maxSearchDepth, 0, Long.MIN_VALUE, Long.MAX_VALUE)
			}
