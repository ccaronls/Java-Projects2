...
import cc.game.zombicide.ZGame.Companion.initDice
import cc.game.zombicide.ZGame.MarksmanComparator
import cc.game.zombicide.ZGame.RangedComparator
import cc.game.zombicide.ZSkillLevel.Companion.getLevel
import cc.game.zombicide.ZSpawnCard.Companion.drawSpawnCard
...
class ZombicideTest : TestCase() {
	...
	fun testLoadBoards() {
		...
		ZQuests.values().forEach { quest ->
			...
			game.board.zones.forEach {
				...
				it.cells.forEach {
					assertFalse(game.board.getCell(it).isCellTypeEmpty)
				}
			}
			game.board.getAllDoors().forEach {
				assertTrue(it.cellPosStart.isIn(game.board.rows, game.board.columns))
				assertTrue(it.cellPosEnd.isIn(game.board.rows, game.board.columns))
			}
		}
	}
	...
	fun testRunGame() = runBlocking {
		...
		for (q in ZQuests.values()) {
			try {
				...
				game.loadQuest(q)
				for (pl in Utils.toList(ZPlayerName.Ariane, ZPlayerName.Clovis, ZPlayerName.Ann, ZPlayerName.Nelly))
				...
				throw Exception("Error running map $q", e)
			}
		}
	}

	class ZTestGame : ZGameRemote() {
	...
	fun testLevels() {
		...
		for (i in 0 until ZColor.YELLOW.dangerPts)
			assertTrue(getLevel(i, rules).color == ZColor.BLUE)
		for (i in ZColor.YELLOW.dangerPts until ZColor.ORANGE.dangerPts)
			assertTrue(getLevel(i, rules).color == ZColor.YELLOW)
		for (i in ZColor.ORANGE.dangerPts until ZColor.RED.dangerPts)
			assertTrue(getLevel(i, rules).color == ZColor.ORANGE)
		for (i in ZColor.RED.dangerPts until 1000)
			assertTrue(getLevel(i, rules).color == ZColor.RED)

		rules.ultraRed = true
		val ultraRed = getLevel(1000, rules)
		assertTrue(ultraRed.isUltra)

		val red = getLevel(ZColor.RED.dangerPts, rules)
