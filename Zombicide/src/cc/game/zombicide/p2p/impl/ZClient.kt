...
import cc.game.zombicide.p2p.CL
import cc.game.zombicide.p2p.CLListener
...
class ZClient(
	...
	override suspend fun notifyListeners(cb: (CLListener) -> Unit) {
		super.notifyListeners {
			(it as? CLListener)?.let {
				cb(it)
			}
		}
	}

	val connectedPlayers: List<IConnectedUser> = TODO()
	/*
	...
	companion object {
		var log = LoggerFactory.getLogger(ZClient::class.java)
	}
