...
import cc.game.zombicide.ZActorPosition
import cc.game.zombicide.ZAnimation
import cc.game.zombicide.ZAttackType
...
import cc.game.zombicide.ZZombieCategory
import cc.game.zombicide.ZZombieType
import cc.game.zombicide.ZZone
import cc.game.zombicide.anims.AscendingAngelDeathAnimation
import cc.game.zombicide.anims.ChargeAttackAnimation
import cc.game.zombicide.anims.DeathAnimation
import cc.game.zombicide.anims.DeathStrikeAnimation
import cc.game.zombicide.anims.DeflectionAnimation
import cc.game.zombicide.anims.EarthquakeAnimation
import cc.game.zombicide.anims.ElectrocutionAnimation
import cc.game.zombicide.anims.EmptyAnimation
import cc.game.zombicide.anims.FadeAnimation
import cc.game.zombicide.anims.FireballAnimation
import cc.game.zombicide.anims.GravestoneAnimation
import cc.game.zombicide.anims.GroupAnimation
import cc.game.zombicide.anims.HandOfGodAnimation
import cc.game.zombicide.anims.InfernoAnimation
import cc.game.zombicide.anims.LightningAnimation2
import cc.game.zombicide.anims.MagicOrbAnimation
import cc.game.zombicide.anims.MakeNoiseAnimation
import cc.game.zombicide.anims.MeleeAnimation
import cc.game.zombicide.anims.MjolnirLightningAnimation
import cc.game.zombicide.anims.MoveAnimation
import cc.game.zombicide.anims.OverlayTextAnimation
import cc.game.zombicide.anims.ShieldBlockAnimation
import cc.game.zombicide.anims.ShootAnimation
import cc.game.zombicide.anims.SlashedAnimation
import cc.game.zombicide.anims.SpawnAnimation
import cc.game.zombicide.anims.StaticAnimation
...
abstract class UIZombicide(
	...
	open fun isGameRunning(): Boolean = gameRunning || client?.connected == true
...
//		boardRenderer.waitForAnimations()
		...
		when (uiMode) {
			...
			UIMode.PICK_DOOR,
			UIMode.PICK_SPAWN -> {
				boardRenderer.pickStackPush(options as List<UIZButton>)
			}

			UIMode.PICK_MENU -> {
				buttonRoot = processMoves(options.filterIsInstance<ZMove>()).takeIf { it.first != null }?.also {
					boardRenderer.pickStackPush(it.getChildren())
				}
...
//				}
	...
	open fun playSound(sound: ZSound, times: Int) {}
	...
	override suspend fun onRollDice(roll: IntArray) {
	...
	private suspend fun animateNecromancerEscapeRoute(necro: ZZombie) {
		necro.getTargetZone(board)?.takeIf { board.isZoneEscapableForNecromancers(it.zoneIndex) }?.let { escapeZone ->
			...
			val spawnRect = escapeZone
				.getEscapeSpawnArea(board) ?: escapeZone
			...
			boardRenderer.addPostActor(object : ZAnimation(500, 6, true) {
				override fun draw(g: AGraphics, position: Float, dt: Float) {
					...
					g.pushMatrix()
					g.translate(spawnRect.center)
					g.scale(1.1f + position * .1f)
					g.translate(spawnRect.center * -1)
					spawnRect.drawOutlined(g)
					g.popMatrix()
				}
			})
			boardRenderer.waitForAnimations()
	...
	override suspend fun onCharacterDefends(cur: ZPlayerName, attackerPosition: ZActorPosition) {
		...
		with(cur.toCharacter()) {
			addAnimation(ShieldBlockAnimation(this))
		}
		boardRenderer.redraw()
	}
	...
	override suspend fun onZombieAttack(zombiePos: ZActorPosition, victim: ZPlayerName, type: ZActionType) {
		...
		boardRenderer.waitForAnimations()
		val charged = CompletableDeferred<Boolean>()
		when (type) {
			ZActionType.MELEE -> {
				zombie.addAnimation(object : ChargeAttackAnimation(zombie, victim.toCharacter()) {
					override fun onCharged() {
						charged.complete(true)
					}
				})
			}
	...
	override suspend fun onCharacterAttacked(
		...
		val victim = character.toCharacter()
		when (attackType) {
			ZAttackType.ELECTROCUTION -> victim.addAnimation(ElectrocutionAnimation(victim))
			/*
			ZAttackType.NORMAL,
			ZAttackType.FIRE,
			...
			ZAttackType.MENTAL_STRIKE -> victim.addAnimation(EarthquakeAnimation(victim, 400))

			else -> victim.addAnimation(SlashedAnimation(victim))
		}
		if (perished) {
			val group = GroupAnimation(victim, true)
			group.addSequentially(AscendingAngelDeathAnimation(victim))
			group.addSequentially(GravestoneAnimation(victim))
			victim.addAnimation(group)
			boardRenderer.waitForAnimations()
		}
		boardRenderer.redraw()
	}
	...
	private suspend fun onAttackMelee(
		...
		if (weapon == ZWeaponType.EARTHQUAKE_HAMMER) {
		...
		} else if (weapon.equipmentClass == ZEquipmentClass.SPEAR && targetZone != attacker.occupiedZone) {
			val animLock = KLock()
			// fade in spear
			// for each thrust, do a oscilating shoot of the spear icon
			// fade out
			val zStart = board.getZone(attacker.occupiedZone)
			val zEnd = board.getZone(targetZone)
			board.getZone(targetZone)
			boardRenderer.pushZoomRect()
			...
			attacker.addAnimation(FadeAnimation(attacker, 700, id))
			var numDice = numDice
			hits.forEach { pos ->
				val victim = board.getActor(pos)
				attacker.addAnimation(object : ThrustAnimation(attacker, victim.center, icon) {
			...
			while (numDice-- > 0) {
				attacker.addAnimation(ThrustAnimation(attacker, zEnd.randomPointInside, icon))
			}

			attacker.addAnimation(object : FadeAnimation(attacker, 700, id) {
				override fun onDone() {
					animLock.release()
				}
			...
			boardRenderer.pushZoomRect()
			boardRenderer.animateZoomTo(board.getZone(attacker.occupiedZone))
			boardRenderer.waitForAnimations()
			for (i in 0 until numDice) {
				if (i < hits.size) {
					...
					playSound(ZSound.SWORD_SLASH, numDice)
					attacker.addAnimation(object : MeleeAnimation(attacker, board) {
						override fun onDone() {
							if (pos.data == ACTOR_POS_DATA_DEFENDED) {
								victim.addAnimation(ShieldBlockAnimation(victim))
	...
	private suspend fun onAttackRanged(
		...
		when (weapon) {
			...
			ZWeaponType.SPEAR -> doShootAnimation(
				attacker,
				numDice,
				hits,
				actionType,
				...
				doShootAnimation(attacker, numDice, hits, actionType, targetZone, ZIcon.ARROW)
			}
		}
	...
	private suspend fun doShootAnimation(
		...
		while (i < numDice) {
			if (i < hits.size) {
				...
				group.addAnimation(
					delay,
					object : ShootAnimation(attacker, 300, victim.center, icon) {
						override fun onDone() {
							super.onDone()
	...
	private suspend fun onAttackMagic(
		...
		when (weapon) {
			...
			ZWeaponType.LIGHTNING_BOLT -> {
				...
				val dir = ZDir.getDirFromOrNull(attacker.occupiedCell, board.getZone(targetZone).cells[0]) ?: ZDir.NORTH
				when (weapon) {
