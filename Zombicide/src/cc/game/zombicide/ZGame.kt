...
import cc.game.zombicide.ZDir.Companion.compassValues
import cc.game.zombicide.ZSpawnCard.ActionType
...
import cc.lib.utils.splitFilter
...
@Remote("game")
abstract class ZGame() : Reflector<ZGame>(), IRemote {
	companion object {
		@JvmField
		var DEBUG = false
...
//            registerClass(ZZombie::class.java)
	...
	open val hoardSize: Int
		get() = board.hoard.map { it.value }.sum()
	...
	private suspend fun updateCurrentUser(idx: Int) {
	...
	open suspend fun pushState(state: State) {
	...
	@RemoteFunction
	protected open suspend fun onCurrentUserUpdated(userName: String, colorId: Int) {
		log.debug("%s updated colorID: $colorId", userName)
	}

	@RemoteFunction
	protected open suspend fun onCurrentCharacterUpdated(priorPlayer: ZPlayerName?, character: ZCharacter?) {
	...
	open fun loadQuest(newQuest: ZQuests) {
		...
		while (it.hasNext()) {
			...
			require(zone.type == ZEnvironmentType.NOTHING || zone.type == cell.environment) { "Zone ${zone.zoneIndex} is not of uniform type" }
			zone.type = cell.environment
	...
	suspend fun spawnZombies(_count: Int, type: ZZombieType, _zone: Int) {
		var zone = _zone
		var count = _count
		...
		for (i in 0 until count) {
			when (type) {
				ZZombieType.SwampTroll -> {
					...
					board.getZonesOfType(ZEnvironmentType.WATER).minByOrNull { wz ->
	...
	private suspend fun performDragonStomp(zoneIdx: Int) {
	...
	@RemoteFunction
	open suspend fun onZombieSpawned(zombie: ZZombie) {
	}

	val state: ZState
		get() = stateStack.peek().state
	...
	@RemoteFunction
	protected open suspend fun onQuestComplete() {
	...
	open suspend fun runGame(): Boolean {
		...
		when (state) {
            ...
            ZState.BEGIN_ROUND -> {
	            ...
	            for (a: ZActor<*> in board.getAllActors())
		            a.onBeginRound(this)
	            repeatableMove = null
            ...
            ZState.SPAWN -> {
	            ...
	            val spawnZones = board.zones.filterIndexed { idx, _ ->
		            board.isZoneSpawnable(idx)
	            }
	            spawnZones.forEachIndexed { index, zZone ->
		            onSpawnZoneSpawning(GRectangle(zZone.getRect()), index, spawnZones.size)
		            spawnZombies(zZone.zoneIndex, highestSkill)
		            onSpawnZoneSpawned(index, spawnZones.size)
	            }
	            board.resetNoise()
            ...
            ZState.PLAYER_STAGE_CHOOSE_CHARACTER_ACTION -> {
	            ...
	            if (rules.familiars && !ch.isStartingFamiliarChosen) {
		            val familiarTypesInPlay = board.getAllActors().filterIsInstance<ZFamiliar>().map {
			            it.type
		            }
		            ZFamiliarType.entries.filter { !familiarTypesInPlay.contains(it) }
			            ...
			            getCurrentUser().chooseFamiliar(ch.type, list)?.let {
				            ZFamiliar(it, ch.type, ch.occupiedZone).apply {
					            ch.addFamiliar(this)
					            board.spawnActor(this)
				            }
			            } ?: return false
		            }
	            ...
	            if (actionsLeft > 0) {
		            ...
		            ch.rangedWeapons.splitFilter { it.isLoaded }.let { (loaded, unloaded) ->
			            loaded.toSet().takeIf { it.isNotEmpty() }?.let { ranged ->
				            options.add(ZMove.newRangedAttackMove(ranged.toList()))
			            }
			            unloaded.forEach {
				            options.add(ZMove.newReloadMove(it))
			            }
		            }
	            ...
	            // player gets all the skills of their familiars that are in same zone
	            board.getCharacterFamiliars(ch.type).filter { it.occupiedZone == ch.occupiedZone }.forEach {
		            it.getAvailableSkills().forEach { skill ->
			            skill.addSpecialMoves(this, ch, options)
		            }
	            }
	...
	private suspend fun performZombieStage() {
		// for this version we take the approach of trying to move to open spaces
		val pathMap = mutableMapOf<ZZombie, MutableList<ZDir>>()

		val zombies = board.getAllZombies().filter { !it.destroyed }.sortedByDescending {
			it.type.targetingPriority // we want the necromancers to move first since they can be followed
		}.toMutableList()
		...
		// perform attacks one at a time so that our attack target sorting
		// logic works
		while (zombies.isNotEmpty()) {
			var numMoved = 0
			zombies.removeAll { it.actionsLeftThisTurn == 0 }
			// find a zombie who can move
			zombies.sortedBy { it.occupiedZone }.forEach { zombie ->
				zombie.clearTargetZone()
				if (tryZombieAttack(zombie)) {
					zombie.actionsLeftThisTurn = 0
					return@forEach
				}

				if (zombie.isEscaping(board)) {
	...
	@RemoteFunction
	protected open suspend fun onCharacterDestroysSpawn(c: ZPlayerName, zoneIdx: Int, area: ZSpawnArea) {
		log.debug("%s destroys spawn at %d", c, zoneIdx, area)
	}

	@RemoteFunction
	protected open suspend fun onCharacterDefends(cur: ZPlayerName, attackerPosition: ZActorPosition) {
		log.debug("%s defends from %s", cur, attackerPosition)
	}

	@RemoteFunction
	protected open suspend fun onNewSkillAcquired(c: ZPlayerName, skill: ZSkill) {
		log.debug("%s acquires new skill %s", c, skill)
	}

	private suspend fun playerDefends(cur: ZCharacter, type: ZZombieType): Boolean {
	...
	private suspend fun tryZombieAttack(zombie: ZZombie): Boolean {
		board.getCharactersInZone(zombie.occupiedZone).filter {
			!it.isInvisible && it.isAlive
		}.sortedWith(WoundingComparator(zombie.type)).firstOrNull()?.let { victim ->
	...
	@RemoteFunction
	protected open suspend fun onZombieAttack(zombiePos: ZActorPosition, victim: ZPlayerName, type: ZActionType) {
	}

	suspend fun gameLost(msg: String) {
	...
	@RemoteFunction
	protected open suspend fun onGameLost() {
		log.debug("GAME LOST")
	}

	suspend fun playerWounded(victim: ZCharacter, attacker: ZActor<*>, attackType: ZAttackType, amount: Int, reason: String) {
		if (!victim.isDead) {
	...
	@RemoteFunction
	protected open suspend fun onCharacterAttacked(
		character: ZPlayerName,
		attackerPosition: ZActorPosition,
		attackType: ZAttackType,
		characterPerished: Boolean
	) {
		log.debug(
			"%s attacked from %s with %s and %s",
			character,
			attackerPosition,
			attackType,
			if (characterPerished) "Died" else "Was Wounded"
		)
	}

	@RemoteFunction
	open suspend fun onEquipmentThrown(c: ZPlayerName, icon: ZIcon, zone: Int) {
		log.debug("%s throws %s into %d", c, icon, zone)
	}

	@RemoteFunction
	protected open suspend fun onStartRound(roundNum: Int) {
	}
	...
	private suspend fun performMove(cur: ZCharacter, _move: ZMove?): Boolean {
		...
		when (move.type) {
			...
			ZMoveType.SWITCH_ACTIVE_CHARACTER -> {
				if (canSwitchActivePlayer()) {
					...
					if (map.isEmpty())
						return false
					((map.indexOfFirst {
						it.first.type == currentCharacter?.type
					} + 1) % map.size).takeIf { it >= 0 }?.let { idx ->
                ...
                when (selectedSlot) {
                    ...
                    ZEquipSlot.RIGHT_HAND, ZEquipSlot.LEFT_HAND, ZEquipSlot.BODY -> {
                        if (!cur.isBackpackFull) {
	                        options.add(ZMove.newUnequippedMove(selectedEquipment, selectedSlot))
                        }
                    }
                }
	        ...
	        ZMoveType.WALK_DIR -> {
		        moveActorInDirection(cur, ZDir.entries[move.integer!!], cur.moveSpeed, move.action)
		        return true
	        }
	        ZMoveType.USE_SLOT -> {
            ...
            ZMoveType.ENCHANT -> {
                when (move.list!!.size) {
	                ...
	                val target = move.character ?: user.chooseCharacterForSpell(cur.type, spell,
		                allLivingCharacters.filter { it.isWounded && board.canSee(cur.occupiedZone, it.occupiedZone) }.map {
			                it.type
		                })
	                target?.let {
	...
	@RemoteFunction
	protected open suspend fun onCatapultFired(
		pl: ZPlayerName,
		fromZone: Int,
		toZone: Int,
		type: ZWeaponType
	...
	private suspend fun performLaunchCatapult(
		...
		with(type.create()) {
			cur.getWeaponStat(this, ZActionType.CATAPULT_FIRE, this@ZGame, zoneIdx)?.let { stat ->
				if (board.getZone(zoneIdx).type == ZEnvironmentType.HOARD) {
	...
	private suspend fun performHoardAttack(zoneIdx: Int, weapon: ZWeaponType) {
		val hoardZombies: MutableList<ZZombieType> =
			board.hoard.map { (type, count) -> Array(count) { type }.toList() }.flatten()
				.sortedBy {
	...
	@RemoteFunction
	protected open suspend fun onZombieHoardAttacked(player: ZPlayerName, hits: List<ZZombieType>) {
	}
	...
	@RemoteFunction
	protected open suspend fun onCloseSpawnArea(c: ZCharacter, zone: Int, area: ZSpawnArea) {
	}

	private suspend fun performCloseSpawnPortal(cur: ZCharacter, move: ZMove): Boolean {
...
/*
	...
	@RemoteFunction
	protected open suspend fun onAhhhhhh(c: ZPlayerName) {
		log.debug("AHHHHHHH!")
	}

	@RemoteFunction
	protected open suspend fun onNecromancerEscaped(position: ZActorPosition) {
		log.debug("necromancer %s escaped", board.getActor(position))
	}

	@RemoteFunction
	protected open suspend fun onEquipmentFound(c: ZPlayerName, equipment: List<ZEquipment<*>>) {
		log.debug("%s found %d eqipments", c, equipment.size)
	}

	@RemoteFunction
	open suspend fun onCharacterHealed(c: ZPlayerName, amt: Int) {
		log.debug("%s healed %d pts", c, amt)
	}

	@RemoteFunction
	protected open suspend fun onSkillKill(c: ZPlayerName, skill: ZSkill, z: ZActorPosition, attackType: ZAttackType) {
		log.debug("%s skill kill on %s with %s", c, board.getActor(z), attackType)
	}

	@RemoteFunction
	open suspend fun onRollSixApplied(c: ZPlayerName, skill: ZSkill) {
		log.debug("%s roll six applied to %s", c, skill)
	}

	@RemoteFunction
	protected open suspend fun onWeaponReloaded(c: ZPlayerName, w: ZWeapon) {
	...
	suspend fun performSkillKill(c: ZSurvivor<*>, skill: ZSkill, z: ZZombie, at: ZAttackType) {
	...
	internal class WoundingComparator(val zType: ZZombieType) : Comparator<ZCharacter> {
		override fun compare(o0: ZCharacter, o1: ZCharacter): Int {
			val v0 = o0.getArmorRating(zType) + o0.woundBar
			val v1 = o1.getArmorRating(zType) + o1.woundBar
	...
	private suspend fun resolveHits(
		...
		var result = if (cur.canReroll(this, stat.attackType)) {
	 ...
	 */
	@RemoteFunction
	protected open suspend fun onAttack(
	...
	@RemoteFunction
	protected open suspend fun onBonusAction(pl: ZPlayerName, action: ZSkill) {
		log.debug("%s got bonus action %s", pl, action)
	}

	private suspend fun checkForHitAndRun(cur: ZSurvivor<*>) {
	...
	suspend fun addExperience(c: ZSurvivor<*>, pts: Int) {
		if (pts <= 0) return
		var sl = c.skillLevel
	...
	@RemoteFunction
	protected open suspend fun onCharacterGainedExperience(c: ZPlayerName, points: Int) {
		log.info("%s gained %d experence!", c, points)
	}

	private suspend fun rollDiceWithRerollOption(numDice: Int, dieNumToHit: Int, _minHitsForAutoReroll: Int, _maxHitsForAutoNoReroll: Int): IntArray {
	...
	suspend fun rollDice(num: Int): IntArray {
		val result = IntArray(num) { dice[it] }
		var dieStrEnd = "+"
		var dieStrMid = "|"
	...
	@RemoteFunction
	protected open suspend fun onRollDice(roll: IntArray) {
		log.info("Rolling dice result is: %s", Arrays.toString(roll))
	}

	suspend fun destroyZombie(
		zombie: ZZombie,
	...
	@RemoteFunction
	protected open suspend fun onZombieDestroyed(deathType: ZAttackType, zombiePos: ZActorPosition) {
	}
	...
	@RemoteFunction
	protected open suspend fun onDoubleSpawn(multiplier: Int) {
		log.debug("Double spawn X %d", multiplier)
	}

	@RemoteFunction
	protected open suspend fun onNothingInSight(zone: Int) {
		addLogMessage("Nothing in Sight")
	}

	private suspend fun doubleSpawn() {
		spawnMultiplier *= 2
		addLogMessage("DOUBLE SPAWN!")
		onDoubleSpawn(spawnMultiplier)
	}

	private suspend fun extraActivation(category: ZZombieCategory) {
	...
	@RemoteFunction
	protected open suspend fun onExtraActivation(category: ZZombieCategory) {
		log.debug("Extra Activation %s", category)
	}

	suspend fun spawnZombies(zoneIdx: Int) {
		spawnZombies(zoneIdx, highestSkillLevel)
	}

	private suspend fun spawnZombies(zoneIdx: Int, level: ZSkillLevel) {
		...
		if (action.action == ActionType.SPAWN && action.types.getOrNull(0)?.isNecromancer == true && !board.canZoneSpawnNecromancers(
				zoneIdx
			)
		) {
		...
		when (action.action) {
			...
			ActionType.SPAWN -> action.types.forEach {
				spawnZombies(action.count, it, zoneIdx)
			}
			...
			ActionType.ASSEMBLE_HOARD -> {
				spawnZombies(action.count, action.types[0], zoneIdx)
				board.addToHoard(action.types[0])
			}
	...
	@RemoteFunction
	protected open suspend fun onSpawnCard(card: ZSpawnCard, color: ZColor) {
	}
	...
	open val currentCharacter: ZCharacter?
		get() = stateStack.peekOrNull()?.player?.toCharacterOrNull()

	val requireCurrentCharacter: ZCharacter
		get() = requireNotNull(currentCharacter)

	protected suspend fun moveActor(actor: ZActor<*>, toZone: Int, speed: Long, actionType: ZActionType?) {
		board.getShortestPath(actor, toZone).forEach {
			moveActorInDirection(actor, it, speed, actionType)
		}
	}

	private suspend fun moveActorPrivate(actor: ZActor<*>, toZone: Int, speed: Long, actionType: ZActionType?) {
	...
	protected open suspend fun moveActorInDirection(actor: ZActor<*>, dir: ZDir, speed: Long, action: ZActionType?) {
		val fromZone = actor.occupiedZone
		val fromPos = actor.occupiedCell
		val fromRect = actor.getRect(board)
		val next = board.getAdjacent(fromPos, dir)
		board.moveActor(actor, next)
		doAnimatedMove(actor, fromZone, fromPos, fromRect, speed, action)
	}

	protected open suspend fun moveActorInDirectionIfPossible(actor: ZActor<*>, dir: ZDir, action: ZActionType?): Boolean {
		val fromZone = actor.occupiedZone
		val fromPos = actor.occupiedCell
		val fromRect = actor.getRect(board)
	...
	private suspend fun doAnimatedMove(
		actor: ZActor<*>,
		fromZone: Int,
		fromPos: Pos,
		fromRect: GRectangle,
		...
		if (board.getZone(fromZone).type === ZEnvironmentType.VAULT && board.getZone(toZone).type !== ZEnvironmentType.VAULT) {
			// ascending the stairs
			val fromVaultRect = GRectangle(board.getCell(fromPos)).scaleEq(.2f)
			val toVaultRect = GRectangle(board.getCell(toPos)).scaleEq(.5f)
		...
		} else if (board.getZone(fromZone).type !== ZEnvironmentType.VAULT && board.getZone(toZone).type === ZEnvironmentType.VAULT) {
			...
			when (board.getCell(fromPos).getWallFlag(board.getDirection(fromZone, toZone))) {
				...
				ZWallFlag.LEDGE -> {
					onActorMoved(actor, fromRect, toRect, speed * 2)
					actor.performAction(
						if (board.getZone(fromZone).type == ZEnvironmentType.WATER)
							ZActionType.CLIMB
						else
	...
	@RemoteFunction
	protected open suspend fun onBeginRound(roundNum: Int) {
		log.debug("Begin round %d", roundNum)
	}

	private suspend fun onActorMoved(actor: ZActor<*>, start: GRectangle, end: GRectangle, speed: Long) {
		onActorMoved(actor.id, start, end, speed)
	}

	@RemoteFunction
	protected open suspend fun onActorMoved(id: String, start: GRectangle, end: GRectangle, speed: Long) {
		log.debug("actor %s moved from %s to %s with speed %d",
			board.getActor(id), start, end, speed)
	}
	...
	suspend fun addNoise(zoneIdx: Int, noise: Int) {
	...
	@RemoteFunction
	protected open suspend fun onNoiseAdded(zoneIndex: Int) {
		log.debug("Noise added at %d", zoneIndex)
	}

	@RemoteFunction
	protected open suspend fun onZombieStageBegin() {
	}

	@RemoteFunction
	protected open suspend fun onZombieStageMoveDone() {
	}

	@RemoteFunction
	protected open suspend fun onZombieStageEnd() {
	}

	protected open suspend fun onZombiePath(id: String, path: List<ZDir>) {}

	@RemoteFunction
	protected open suspend fun onSpawnZoneSpawning(rect: GRectangle, nth: Int, num: Int) {
	}

	@RemoteFunction
	protected open suspend fun onSpawnZoneSpawned(nth: Int, num: Int) {
	}
	...
	@RemoteFunction
	open suspend fun onIronRain(c: ZPlayerName, targetZone: Int) {
		log.debug("%s unleashed iron rain at %d", c, targetZone)
	}

	@RemoteFunction
	protected open suspend fun onDoorUnlocked(door: ZDoor) {
	...
	open suspend fun addLogMessage(msg: String) {
		log.info(msg)
	}

	suspend fun performDragonFire(cur: ZSurvivor<*>, zoneIdx: Int) {
	...
	suspend fun giftEquipment(c: ZCharacter, e: ZEquipment<*>) {
	...
	private suspend fun tryOpenDoor(cur: ZCharacter, door: ZDoor): Boolean {
	...
	@RemoteFunction
	protected open suspend fun onDragonBileExploded(zoneIdx: Int) {
		log.debug("Dragon bil exploded in %d", zoneIdx)
	}

	suspend fun chooseEquipmentFromSearchables() {
		pushState(State(ZState.PLAYER_STAGE_CHOOSE_WEAPON_FROM_DECK, currentCharacter?.type))
	}

	suspend fun chooseVaultItem() {
		pushState(State(ZState.PLAYER_STAGE_CHOOSE_VAULT_ITEM, currentCharacter?.type))
	}

	suspend fun giftRandomVaultArtifact(c: ZCharacter) {
	...
	fun setUserName(user: ZUser, name: String?) {
		user.name = name ?: ZUser.getColorName(user.colorId)
	}
	...
	@RemoteFunction
	open suspend fun setBoardMessage(colorId: Int, message: String?) {
	}
