...
class ZWeapon(override val type: ZWeaponType=ZWeaponType.AXE) : ZEquipment<ZWeaponType>() {
    ...
    fun fireWeapon(game: ZGame, cur: ZCharacter, stat: ZWeaponStat) {
	    if (type.equipmentClass.needsReload)
		    isEmpty = true
	    if (type.equipmentClass.isOneShot) {
	...
	fun getComparisonInfo(game : ZGame, c0 : ZCharacter, c1 : ZCharacter) : Table {
		...
		for (at in ZActionType.entries) {
			listOfNotNull(
				...
				val hitPercent =
					getComparisonArray(c0Stats.numDice, c1Stats.numDice) { "${c0Stats.dieRollToHitPercent}% x $it" }
				val range = getComparisonArray(c0Stats.maxRange, c1Stats.maxRange) { "$it" }
				val dieRollToOpenDoor =
					getComparisonArray(c0Stats.dieRollToOpenDoorPercent, c1Stats.dieRollToOpenDoorPercent) { if (it == 0) "no" else "$it%" }
				val equippable =
					getComparisonArray(isEquippable(c0).toInt(), isEquippable(c1).toInt()) { if (it == 0) "no" else "yes" }
		    ...
		    c.getWeaponStat(this, at, game, -1)?.let { stats ->
	            ...
	            cardLower.addColumnNoHeader(
		            listOf(
			            ...
			            doorInfo,
			            if (type.equipmentClass.needsReload) String.format(
				            "yes (%s)",
