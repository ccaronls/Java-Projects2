package cc.game.kaiser.console;

import java.io.*;

import cc.game.kaiser.ai.PlayerBot;
import cc.game.kaiser.console.KaiserConsole;
import cc.game.kaiser.core.*;
import cc.game.kaiser.net.KaiserClient;
import cc.game.kaiser.net.KaiserServer;
import cc.game.kaiser.net.KaiserServerExample;
...
public class KaiserConsole { 
    ...
    void chooseGameType() {
        ...
        while (true) {
            ...
            switch (option.charAt(0)) {
                ...
                case 'H': hostMultiPlayerGame(name); break;
                case 'J': joinMultiPlayerGame(name); break;
    ...
    void hostMultiPlayerGame(String name) {

        printf("starting kaiser server ...");
        try {
            ...
            while (running) {
                ...
                try {
                    ...
                    switch (num) {
                        case 1: {
                            for (ClientConnection conn: server.getServer().getConnectionValues()) {
                                printf("Client %-20s:   connected(%b) \n", conn.getName(), conn.isConnected());
                            }
                            break;
                        }
                        case 2: {
                            this.drawGame(server);
                            break;
                        }
                        case 3: {
    ...
    void joinMultiPlayerGame(String name) {
        
        String server = null;
        printf("enter server address or nothing for localhost>");
        server = readLine().trim();
        if (server.length() <= 0)
            server = "localhost";
        KaiserClient client = new KaiserClient(new ConsolePlayer(name), KaiserServerExample.VERSION) 
        {

            @Override
            protected void drawGame(Kaiser kaiser) {
                KaiserConsole.this.drawGame(kaiser);
            }
        ...
        try {
            client.connect(server, KaiserServerExample.PORT);
            synchronized (client) {
                client.wait();
            }
        } catch (Exception e) {
