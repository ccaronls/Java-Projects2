...
abstract class UIGeniusSquares : GeniusSquares() {
	...
	fun paint(g: APGraphics, mx: Int, my: Int) {
		...
		if (WIDTH > HEIGHT) {
			...
			g.popMatrix()
			g.setTextHeight(CELL_DIM / 2, true)
			timer.capture()
			...
			pickedCell?.takeIf { false }?.let { pickedCell ->
				...
				g.drawJustifiedString(WIDTH - 10, 10f, Justify.RIGHT, Justify.TOP, hl + "pickedCell: " + pickedCell[0] + "," + pickedCell[1])
			}
		} else {
	...
	inner class EndGameAnim : AAnimation<AGraphics, EndGameAnim>(2000, 1, true) {
		override fun draw(g: AGraphics, position: Float, dt: Float) {
			...
			g.drawJustifiedString(WIDTH / 2, HEIGHT / 2, Justify.CENTER, Justify.CENTER, "COMPLETED")
		}
	}
