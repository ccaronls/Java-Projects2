...
class PlayerUser : Player() {
	...
	override suspend fun chooseMove(game: Monopoly, options: List<MoveType>): MoveType? {
		return (game as UIMonopoly).showChooseMoveMenu(this, options)
	}

	override suspend fun chooseCard(game: Monopoly, cards: List<Card>, type: CardChoiceType): Card? {
		return (game as UIMonopoly).showChooseCardMenu(this, cards, type)
	}

	override suspend fun chooseTrade(game: Monopoly, trades: List<Trade>): Trade? {
		return (game as UIMonopoly).showChooseTradeMenu(this, trades)
	}

	override suspend fun markCardsForSale(game: Monopoly, sellable: List<Card>): Boolean {
