	...
	override suspend fun onPlayerOutOfJail(playerNum: Int) {
		setSpriteAnim("PLAYER$playerNum", object : SpriteAnimation(500) {
			...
			override fun draw(g: Sprite, position: Float, dt: Float) {
				g.M.translated(start.add(end.sub(start).scaledBy(position)))
			}
		}.start())
				...
				spriteMap[sq.name] = object : Sprite() {
					override fun draw(g: AGraphics, w: Float, h: Float) {
						...
						when (board.getSquarePosition(sq)) {
							Board.Position.TOP -> {
								Pair(Vector2D(r.left + r.width / 2, r.top + r.height - houseScale), 0)
							}

							Board.Position.RIGHT -> {
								Pair(Vector2D(r.left + houseScale, r.top + r.height / 2), 270)
							}

							Board.Position.BOTTOM -> {
								Pair(Vector2D(r.left + r.width / 2, r.top + houseScale), 0)
							}

							Board.Position.LEFT -> {
								Pair(Vector2D(r.left + r.width - houseScale, r.top + r.height / 2), 90)
							}
