...
public class KaiserApplet extends AWTKeyboardAnimationApplet {
    ...
    private void drawTricksTaken(AGraphics g) {
        ...
        for (int i=0; i<kaiser.getNumPlayers(); i++) {
            ...
            for (int ii=0; ii<p.getNumTricks(); ii++) {
                ...
                final int maxLenPixels = Math.round(cardWidth*2);
    ...
    private void drawGame(AGraphics g) {
        synchronized (this) {
            ...
            final int fh = Math.round(g.getTextHeight() + 3);
            ...
            final float d = cardWidth/2;
    
            final float [] tdx = { -d, d, -d, -(d+cardHeight) };
            float [] tdy = { d, -d, -(d+cardHeight), -d };
    
            final int maxCardsWidth = screenHeight/2;
            
            final int txtPadding = 15;
            final float [] nx = { screenHeight-txtPadding, screenHeight-txtPadding, txtPadding, txtPadding };
            final float [] ny = { y[0], txtPadding, txtPadding, y[0] };
            ...
            for (int i=0; i<4; i++) {
                drawPlayerHand(g, getPlayer(i), Math.round(x[i]), Math.round(y[i]), Angle.values()[i], maxCardsWidth);
                ...
                if (trick != null) {
                    this.drawCard(g, trick, Math.round(c+tdx[i]), Math.round(c+tdy[i]), Angle.values()[i], true);
                }
            }
