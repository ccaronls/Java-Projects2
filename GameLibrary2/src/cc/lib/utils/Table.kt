...
class Table(var model: Model = object : Model {}) : ITableItem, IButton {
	...
	fun addColumnNoHeader(items: Array<*>): Table {
		return addColumnNoHeader(items.toList())
	}
	...
	override fun measure(g: AGraphics): GDimension {
		...
		for (r in rows.indices) {
			for (c in rows[r].indices) {
				...
				if (o is ITableItem) {
					...
					val parts = entry.wrap(model.getMaxCharsPerLine())
					for (s in parts) {
		...
		var dimWidth = maxWidth.sum()
		var dimHeight = maxHeight.sum() + headerHeight
	...
	fun toString(indent: Int): String {
		...
		val paddingChars = " ".repeat(padding)
		...
		val headerPadding = "-".repeat(padding)
		...
		for (i in 0 until columns - 1) {
			buf.append("-".repeat(maxWidth[i])).append(headerDivMid)
		}
		val last = columns - 1
		buf.append("-".repeat(maxWidth[last])).append(headerDivEnd)
			...
			for (ii in 0 until headerHeightLines) {
				...
				if (last < header.size) {
					...
					if (ii < parts.size)
						...
						buf.append(" ".repeat(maxWidth[last]))
				} else {
					buf.append(" ".repeat(maxWidth[last]))
				}
		...
		totalWidthChars = maxWidth.sum() + (this.columns - 1) * (padding * 2 + 1) + if (border) 2 + padding * 2 else 0
		totalHeightChars = 1
	...
	private fun getJustifiedString(s: String, justify: Justify, cellWidth: Int): String {
		when (justify) {
			...
			Justify.CENTER -> {
				...
				return String.format("%" + frontWidth + "s", s) + " ".repeat(backPadding)
			}
