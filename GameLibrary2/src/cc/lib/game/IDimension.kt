...
interface IDimension {
	...
	fun rotated(degrees: Float): GDimension? {
		val tl: Vector2D = Vector2D(-width / 2, -height / 2).rotated(degrees)
		val tr: Vector2D = Vector2D(width / 2, -height / 2).rotated(degrees)
		val newWidth = abs(tl.x).coerceAtLeast(abs(tr.x)) * 2
		val newHeight = abs(tl.y).coerceAtLeast(abs(tr.y)) * 2
	...
	fun addVert(d: GDimension): GDimension? {
		return GDimension(width.coerceAtLeast(d.width), height + d.height)
	}

	fun addHorz(d: GDimension): GDimension? {
		return GDimension(width + d.width, height.coerceAtLeast(d.height))
	}
