...
abstract class AMultiPhaseAnimation<T> : AAnimation<T> {
	...
	protected constructor(durations: LongArray) : super(durations.sum()) {
		this.durations = durations
	}

	protected constructor(duration: Long) : super(duration) {
		durations = longArrayOf(duration)
	}

	constructor(durationMSecs: Long, repeats: Int) : super(durationMSecs, repeats) {
		durations = longArrayOf(durationMSecs)
	}

	constructor(durationMSecs: Long, repeats: Int, oscillateOnRepeat: Boolean) : super(
		durationMSecs,
		repeats,
		oscillateOnRepeat
	) {
