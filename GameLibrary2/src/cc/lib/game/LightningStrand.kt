...
class LightningStrand(
	val startInt: IInterpolator<Vector2D>,
	val endInt: IInterpolator<Vector2D>,
	val arcInt: IInterpolator<Float>,
	val minSections: Int,
	val maxSections: Int,
	val excitability: Float
) {
	constructor(
		startInt: IInterpolator<Vector2D>,
		endInt: IInterpolator<Vector2D>,
		minSections: Int,
		maxSections: Int,
		excitability: Float
...
*/
	...
	companion object {
		fun drawLightning(
			...
			val sec = randRange(minSections, maxSections)
			if (sec < 1) return
			val start = _start.toMutable()
			val vInt = Bezier.build(start, end.toMutable(), arcBend)
			g.color = GColor.WHITE.withAlpha(.5f + .5f.randomPositive())
			g.setLineWidth(randRange(1, 5).toFloat())
			g.begin()
			g.vertex(start)
			var sign = 1f
			for (ii in 1..sec) {
				...
				val dv: Vector2D = v.minus(start).rotateEq(sign * (30 * excitability).randomPositive())
				sign *= -1f
				g.vertex(start.addEq(dv))
				//start.set(v);
			}
