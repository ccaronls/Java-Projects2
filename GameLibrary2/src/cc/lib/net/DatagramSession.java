...
abstract public class DatagramSession {
    ...
    public synchronized long write(long userData, byte[] message, int size, int priority) throws IOException {
        ...
        while (mSocket != null & packetIndex < num) {
            ...
            if (DEBUG_ENABLED)
                printDebug("WRITER: writeHeader id(" + id + ") size (" + size + ") index (" + packetIndex + ") num (" + num + ")");
            //    private static void writeHeader(DataOutputStream out, long id, int priority, int size, int packetIndex, int num, long userData) throws IOException {
            ...
            try {
                ...
                if (simulateDroppedPackets) {
                    if (id > 1 && Math.random() > 0.5) {
                        ...
                        mBuffer.reset();
                        if (DEBUG_ENABLED)
                            printDebug("WRITER: *SIMDROP* id " + id + ", index '" + packetIndex + "'");
                        continue;
                    }
                }
                    ...
                    new Thread(new Runnable() {
                        public void run() {
                            try {
                                ...
                                if (DEBUG_ENABLED)
                                    printDebug("WRITER: *SIMDELAY* id '" + id + "' index '" + indexToSend + "'");
        ...
        void process(DatagramPacket packet) throws Exception {
            ...
            do { // sumptin to break out uv
                ...
                if (packet.getAddress() == null) {
                    ...
                    if (DEBUG_ENABLED)
                        printDebug("Ignoring packet from '" + packet.getAddress() + "' since we are connected to  '" + mAddress + "'");
                    break;
                }
                ...
                if (DEBUG_ENABLED)
                    printDebug("READER: Read header id(" + id + ") priority (" + priority + ") size (" + size + ") index (" + index + ") num (" + num + "}");
                ...
                if (id < minTrackingId) {
                    // ignore
                    if (DEBUG_ENABLED)
                        printDebug("READER: Ignoring packet '" + id + "' since the min is '" + minTrackingId + "'");
                    break;
                }
                ...
                while (insertIndex >= mMaxRangePackets) {
                    if (DEBUG_ENABLED)
                        printDebug("READER: Flushing packet '" + minTrackingId + "'");
                    doCallback();
                    insertIndex -= 1;
                }
                ...
                if (size > 0) {
                    if (DEBUG_ENABLED)
                        printDebug("READER: Inserting id '" + id + "' index '" + insertIndex + "' with min '" + minTrackingId + "'");
