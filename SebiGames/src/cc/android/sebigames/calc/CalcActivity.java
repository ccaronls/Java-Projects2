...
public class CalcActivity extends CCActivityBase implements OnClickListener, OnInitListener {
	...
	protected void onCreate(Bundle b) {
        ...
        if (getChildsName().isEmpty())
            showChildsNameDialog();
	}
	...
	void showInfoDialog() {
        ...
        String [] items = {
                "Voice " + (isQuiet() ? "On" : "Off"),
                "Child's Name " + getChildsName()
        };
    ...
    private void showChildsNameDialog() {
        final EditText et = new EditText(this);
        et.setText(getChildsName());
        et.setSelection(0);
        new AlertDialog.Builder(this).setTitle("Update Childs Name").setView(et).setNegativeButton("Cancel", null)
                .setPositiveButton("Ok", new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialogInterface, int i) {
                        setChildsName(et.getText().toString());
                    }
                }).show();
    }

    private boolean isQuiet() {
        return getPrefs().getBoolean("PREF_CALC_QUIET", true);
    }

    private void setQuiet(boolean quiet) {
        getPrefs().edit().putBoolean("PREF_CALC_QUIET", quiet).apply();
    }

    private String getChildsName() {
        return getPrefs().getString("PREF_CHILDS_NAME", "");
    }

    private void setChildsName(String name) {
        getPrefs().edit().putString("PREF_CHILDS_NAME", name).apply();
    }
}
